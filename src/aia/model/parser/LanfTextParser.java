 ...
 * @author Ratino
 */
public class LanfTextParser implements BaseTextParserInterface{

    @Override
    public BaseModel parse(String[] c) {
        LanfModel model = new LanfModel();
        ...
        model.setTopupVi(c[44]);
        model.setCabLb(c[45]);
        model.setFldent(c[46]);
        model.setYbankkey(c[47]);
        model.setYsustyp(c[48]);
        model.setRekAia(c[49]);
        model.setVaOwner(c[50]);
        model.setFlgEstate(c[51]);
        model.setCltEmail(c[52]);
        model.setDteBirth(c[53]);
